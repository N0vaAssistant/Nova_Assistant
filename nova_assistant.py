...
def init_groq_client():
    ...
    if GROQ_API_KEY:
        ...
        print("⚠️ GROQ_API_KEY non configurée. Les fonctions IA ne seront pas disponibles.")
...
def synthese_vocale(texte):
    if not ELEVEN_API_KEY:
        print("⚠️ ELEVENLABS_API_KEY non configurée. La synthèse vocale est désactivée.")
    ...
    try:
        ...
        if not response.content:
            print("⚠️ Réponse audio vide de ElevenLabs.")
            return
...
<head>
    ...
    <title>Nova Assistant Configuration</title>
    <link rel="stylesheet" href="/style.css">
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700&display=swap" rel="stylesheet">
</head>
<body>
    <div class="container">
        <h1>Nova Assistant Configuration</h1>
        <p class="description">Configurez le comportement de votre assistant vocal local.</p>

        <div class="card">
            <div class="card-header">
                <h2>Paramètres de Nova</h2>
            </div>
            <div class="card-content">
                <div class="status-section">
                    <span>Statut de l'Assistant Local :</span>
                ...
                <div class="form-group">
                    <label for="mode-select">Mode de Conversation</label>
                    <select id="mode-select">
                        <option value="rapide">Rapide (clair, précis, droit au but)</option>
                        <option value="normal">Normal (par défaut)</option>
                        <option value="histoire">Histoire (raconte des anecdotes, parle beaucoup)</option>
                    </select>
                </div>

                <div class="form-group">
                    <label for="hotword-input">Mot-clé d'activation (Hotword)</label>
                    <input type="text" id="hotword-input" value="nova" disabled class="disabled-input">
                    <p class="hint">Le hotword est configuré via les variables d'environnement.</p>
                </div>
            </div>
            <div class="card-footer">
                <button id="apply-config-btn">Appliquer la Configuration</button>
            </div>
        </div>
    </div>
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const LOCAL_ASSISTANT_URL = "http://127.0.0.1:5000"; // URL du serveur Flask local

  // Function to update connection status in the UI
  function updateConnectionStatus(status) {
    connectionStatusSpan.className = `status ${status}`;
    if (status === "connecting") {
      connectionStatusSpan.innerHTML = '<span class="spinner"></span> Connexion...';
    } else if (status === "connected") {
      connectionStatusSpan.innerHTML =
        '<svg class="icon" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clip-rule="evenodd"></path></svg> Connecté';
    } else {
      connectionStatusSpan.innerHTML =
  ...
  // Function to check connection and sync status/mode from local assistant
  async function checkConnectionAndSync() {
    updateConnectionStatus("connecting");
    try {
      const response = await fetch(`${LOCAL_ASSISTANT_URL}/status`, { cache: "no-store" });
      if (response.ok) {
        const data = await response.json();
        modeSelect.value = data.current_mode || "normal";
        hotwordInput.value = data.hotword || "nova";
        updateConnectionStatus("connected");
      } else {
        updateConnectionStatus("disconnected");
      }
    } catch (error) {
      console.error("Failed to connect to local assistant:", error);
      updateConnectionStatus("disconnected");
    }
  }

  // Function to apply configuration to the local assistant
  applyConfigBtn.addEventListener("click", async () => {
    const selectedMode = modeSelect.value;
    try {
      const response = await fetch(`${LOCAL_ASSISTANT_URL}/config`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
